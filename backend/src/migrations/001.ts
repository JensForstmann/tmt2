...
import { isMigrationDone, setMigrationDone } from './migrations';

const STORAGE_FOLDER = process.env['TMT_STORAGE_FOLDER'] || 'storage';

export const migration001 = () => {
	if (isMigrationDone(__filename)) {
		return;
	}
	console.info('run migration001');
	...
	migrateEvents();
	migrateLogs();
	createMatchPlayerEventTables();

	setMigrationDone(__filename);
};
...
const migrateMatches = () => {
	...
	const insertMatchPlayerStatement = db.prepare(
		...
		try {
			...
			(match.players as any[]).forEach((player) => {
				const params = {
					...
					isBot: player.steamId64.startsWith('BOT_') ? 1 : 0,
				};
...
const createMatchPlayerEventTables = () => {
	db.prepare(
		`CREATE TABLE IF NOT EXISTS matchPlayerEventRound (
            timestamp TEXT NOT NULL,
            matchId TEXT NOT NULL,
            mapIndex INTEGER NOT NULL,
            roundNumber INTEGER NOT NULL,
            playerSteamId64 TEXT NOT NULL,
            PRIMARY KEY (matchId, mapIndex, roundNumber, playerSteamId64)
            FOREIGN KEY (matchId, mapIndex) REFERENCES matchMap (matchId, "index") ON UPDATE CASCADE ON DELETE CASCADE
        ) STRICT`
	).run();
	db.prepare(
		`CREATE TABLE IF NOT EXISTS matchPlayerEventAttack (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT NOT NULL,
            matchId TEXT NOT NULL,
            mapIndex INTEGER NOT NULL,
            ...
            damageArmor INTEGER NOT NULL,
            health INTEGER NOT NULL,
            armor INTEGER NOT NULL,
            hitGroup TEXT NOT NULL,
            isFriendlyFire INTEGER NOT NULL,
            FOREIGN KEY (matchId, mapIndex) REFERENCES matchMap (matchId, "index") ON UPDATE CASCADE ON DELETE CASCADE
        ) STRICT`
	).run();
	db.prepare(
		`CREATE TABLE IF NOT EXISTS matchPlayerEventKill (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT NOT NULL,
            matchId TEXT NOT NULL,
            mapIndex INTEGER NOT NULL,
            ...
            attackerInAir INTEGER NOT NULL,
            isFriendlyFire INTEGER NOT NULL,
            FOREIGN KEY (matchId, mapIndex) REFERENCES matchMap (matchId, "index") ON UPDATE CASCADE ON DELETE CASCADE
        ) STRICT`
	).run();
	db.prepare(
		`CREATE TABLE IF NOT EXISTS matchPlayerEventSuicide (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT NOT NULL,
            matchId TEXT NOT NULL,
            mapIndex INTEGER NOT NULL,
	...
	db.prepare(
		`CREATE TABLE IF NOT EXISTS matchPlayerEventAssist (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT NOT NULL,
            matchId TEXT NOT NULL,
            mapIndex INTEGER NOT NULL,
	...
	db.prepare(
		`CREATE TABLE IF NOT EXISTS matchPlayerEventBlind (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT NOT NULL,
            matchId TEXT NOT NULL,
            mapIndex INTEGER NOT NULL,
	...
	db.prepare(
		`CREATE TABLE IF NOT EXISTS matchPlayerStats (
            ...
            enemiesFlashed INTEGER NOT NULL,
            averageDamagePerRound INTEGER NOT NULL,
            health INTEGER NOT NULL,
            armor INTEGER NOT NULL,
            headshots INTEGER NOT NULL,
            items TEXT NOT NULL,
            PRIMARY KEY (matchId, mapIndex, steamId64)
            FOREIGN KEY (matchId, mapIndex) REFERENCES matchMap (matchId, "index") ON UPDATE CASCADE ON DELETE CASCADE
        ) STRICT`
	).run();
};
