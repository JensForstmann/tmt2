...
export const setup = async () => {
    ...
    const playerMatchStatsAttributes = [
        { name: 'steamId', type: 'TEXT', constraints: `REFERENCES ${PLAYERS_TABLE}(steamId)` },
        { name: 'matchId', type: 'TEXT', constraints: `REFERENCES ${MATCHES_TABLE}(matchId)` },
        { name: 'kills', type: 'INTEGER' },
        { name: 'deaths', type: 'INTEGER' },
        { name: 'assists', type: 'INTEGER' },
        { name: 'diff', type: 'INTEGER' },
        { name: 'headshots', type: 'FLOAT', constraints: 'CHECK (headshots >= 0 AND headshots <= 100)' },
        { name: 'adr', type: 'FLOAT' }
    ] as SqlAttribute[];
    const playerMatchStatsTableSchema = new TableSchema(PLAYER_MATCH_STATS_TABLE, playerMatchStatsAttributes, ['steamId', 'matchId']);
